...
function displayWeatherInfo(data) {
    ...
    if (fiveDayForecast.length > 0) {
        ...
        const avgTemp = (today.temps.reduce((a, b) => a + b, 0) / today.temps.length).toFixed(1);
        const avgHumidity = (today.humidities.reduce((a, b) => a + b, 0) / today.humidities.length).toFixed(1);
        const avgWindSpeed = (today.windSpeeds.reduce((a, b) => a + b, 0) / today.windSpeeds.length).toFixed(1);
